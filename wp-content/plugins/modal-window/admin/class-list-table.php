...
class Wow_List_Table extends WP_List_Table {
	...
	public function column_title( $item ) {
		...
		$text          = 'modal-window';
		$title         = ! empty( $item['title'] ) ? $item['title'] : esc_attr__( 'Untitle', 'modal-window' );
		...
		$actions       = array(
			'edit'      => '<a href="' . esc_url( $edit_url ) . '">' . esc_attr__( 'Edit', 'modal-window' ) . '</a>',
			'duplicate' => '<a href="' . esc_url( $duplicate_url ) . '" class="has-text-success">' . esc_attr__( 'Duplicate', 'modal-window' )
			               . '</a>',
			'delete'    => '<a href="' . esc_url( wp_nonce_url( $delete_url, $slug . '_nonce' ) ) . '" class="has-text-danger">' . esc_attr__( 'Delete', 'modal-window' ) . '</a>',
		);
	...
	public function get_columns() {
		$columns = array(
			...
			'title'  => esc_attr__( 'Title', 'modal-window' ),
			'code'   => esc_attr__( 'Shortcode', 'modal-window' ),
			'test'   => esc_attr__( 'Test Mode', 'modal-window' ),
			'status' => esc_attr__( 'Status', 'modal-window' ),
		);
	...
	private function table_data() {
		...
		if ( $result ) {
			foreach ( $result as $key => $value ) {
				...
				$test_on        = '<span class="has-text-danger has-text-weight-semibold">' . esc_attr__( 'On', 'modal-window' ) . '</span>';
				$test_off       = esc_attr__( 'Off', 'modal-window' );
				$test           = ! empty( $param['test_mode'] ) ? $test_on : $test_off;
				$tooltip_off    = esc_attr__( 'Click for Deactivate the popup.', 'modal-window' );
				$tooltip_on     = esc_attr__( 'Click for Activate the popup.', 'modal-window' );
				...
				$status_on      = '<a href="' . esc_url( wp_nonce_url( $deactivate_url, $slug . '_nonce' ) ) . '" class="button is-info is-small has-tooltip-bottom" data-tooltip="' . esc_attr( $tooltip_off ) . '">' . esc_attr__( 'Activated', 'modal-window' ) . '</a>';
				$status_off     = '<a href="' . esc_url( wp_nonce_url( $activate_url, $slug . '_nonce' ) ) . '" class="button is-danger is-small has-tooltip-bottom" data-tooltip="' . esc_attr( $tooltip_on ) . '">' . esc_attr__( 'Deactivated', 'modal-window' ) . '</a>';
				$status         = ! empty( $value->status ) ? $status_on : $status_off;
				$title          = ! empty( $value->title ) ? $value->title : esc_attr__( 'Untitle', 'modal-window' );
				$data[]         = array(
					'ID'    => $value->id,
	...
	public function get_bulk_actions() {
		$actions = array(
			'delete-items'     => esc_attr__( 'Delate', 'modal-window' ),
			'activate-items'   => esc_attr__( 'Activate', 'modal-window' ),
			'deactivate-items' => esc_attr__( 'Deactivate', 'modal-window' ),
		);
